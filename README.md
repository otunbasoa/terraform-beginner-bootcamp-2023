...
We need to be careful when using the Init because it will not rerun if we restart an existing workspace. so we replaced `init` with `before`
...
```json
{
    "UserId": "AIEA7ASCFW4TA2MS32IEX",
    "Account": "123456789012",
    "Arn": "arn:aws:iam::123456789012:user/altschooluser"
}
```
...
To create a more seamless flow of connecting to our terraform cloud, we automated the process using bashscript, see code below 
...
then ran `./bin/generate_tfr_credentials`
